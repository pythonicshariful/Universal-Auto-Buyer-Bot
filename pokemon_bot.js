...
const API_URL = `${API_HOST}/api/pokemon/products`;
...
async function sendWebhook(title, productData, color = 0x00ff00) {
    ...
    try {
        ...
        console.log("Pokemon Webhook delivered successfully.");
...
async function handleProduct(productUrl, page) {
    console.log(`Checking Pokemon: ${productUrl}`);
    ...
    try {
        ...
        if (['robot', 'human', 'verify', 'captcha', 'security'].some(s => pageTitle.toLowerCase().includes(s))) {
            console.log('  [ALERT] Pokemon Bot detection page! Skipping this cycle.');
            return;
        }
        ...
        const dom = await page.evaluate(() => {
            ...
            // Price
            const priceEl = document.querySelector('.product-price') || document.querySelector('span[class*="price"]');
        ...
        console.log(`[Pokemon] ${productData.name} | $${productData.price} | Stock: ${productData.in_stock}`);
        ...
        if (!lastData) {
            console.log(`[STATE CHANGE] ✨ NEW POKEMON LISTING ✨ - ${productData.name}`);
            await sendWebhook('✨ NEW POKEMON LISTING ✨', productData, 0xef5350);
            ...
            if (!lastData.in_stock && productData.in_stock) {
                webhookTitle = '🚨 POKEMON RESTOCK DETECTED 🚨'; color = 0x00ff00;
            } else if (lastData.in_stock && !productData.in_stock) {
                webhookTitle = '⚠️ POKEMON OUT OF STOCK ⚠️'; color = 0xff0000;
            } else if (lastData.price !== productData.price && productData.price) {
                webhookTitle = '💰 POKEMON PRICE CHANGED 💰'; color = 0xffff00;
            }
        ...
        console.error(`Error checking Pokemon ${productUrl}:`, e.message);
    }
}
...
async function runBot() {
    console.log("Starting Pokemon Monitor...");
    await fetchSettings();
    let checkCount = 0;
    ...
    while (true) {
        ...
        if (products.length === 0) {
            console.log("No Pokemon products to monitor in database. Waiting...");
            await sleep(5000);
            continue;
        }
